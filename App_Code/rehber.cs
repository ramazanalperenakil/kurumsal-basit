...
public class rehber
{
    ...
    {
        SqlConnection baglanti = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["ConnectionString2A"].ConnectionString);
        baglanti.Open();
        return baglanti;
    ...
    {
        SqlConnection baglanti = this.baglan();
        SqlDataAdapter adaptor = new SqlDataAdapter(sqlCumle, baglanti);
    ...
    public string Tirnak(string Text)
    {
    ...
    public string KartKaydet(FileUpload fu, int sonGenislik, int sonYukseklik, string gercekYol, string onAd)
    {
        string yoluKaydet;
